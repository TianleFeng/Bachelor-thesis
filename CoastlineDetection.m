...
%%
counter=1;
for a=120:370                                          %amplitude threshold
    cl=Coastline(fcombieeg,100);                       %cl=Coastline(EEGdata,halfsr)
    cl(abs(cl)>a)=nan;                         
    cl(1)=nan;
    nsupp=find(isnan(cl));
    diffns=diff(nsupp);
    ...
    evatable_cl = [amp_th_uV FPratio FNratio sensi speci qual];
    
    counter=counter+1;
end
%%
col=cellstr(["amp_th_uV" "FPratio" "FNratio" "sensi" "speci" "qual"]);
T=array2table(evatable_cl, 'VariableNames', col);
[~,maxidx]=max(T.qual);
