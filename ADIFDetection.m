load('verified_goldstd_5.nmd','-mat') 
...
counter=1;
for a=540:590                                       %amplitude threshold
    adif=ADIF(combieeg,200,100,8);                  %adif=ADIF(EEGdata,sr,halfsr,locutoff)
    adif(abs(adif)>a)=nan;                         
    adif(1)=nan;
    nsupp=find(isnan(adif));
    diffns=diff(nsupp);
    suppsec=find(diffns>=200);                      %if the length between 2 NaNs >=1 sec will be detected as suppression
    suppvect=zeros(length(combieeg),1);
    ...
    evatable_adif = [amp_th_uV FPratio FNratio sensi speci qual];
    
    counter=counter+1;
end

col=cellstr(["amp_th_uV" "FPratio" "FNratio" "sensi" "speci" "qual"]);
T=array2table(evatable_adif, 'VariableNames', col);
[~,maxidx]=max(T.qual);
